...
namespace Chrio.Controls
{
    public class CameraZoomController : SharksBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Keyboard.current.shiftKey.isPressed) Speed = 20; else Speed = 10;
            ...
            if (Keyboard.current.aKey.isPressed || Keyboard.current.dKey.isPressed)
                _camera.transform.position = _camera.transform.position.UpdateAxisEuler(_camera.transform.position.x + ExtraFunctions.GetAxis("Horizontal") * Time.deltaTime * Speed, ExtraFunctions.Axis.X);
            if (Keyboard.current.wKey.isPressed || Keyboard.current.sKey.isPressed)
                _camera.transform.position = _camera.transform.position.UpdateAxisEuler(_camera.transform.position.y + ExtraFunctions.GetAxis("Vertical") * Time.deltaTime * Speed, ExtraFunctions.Axis.Y);
